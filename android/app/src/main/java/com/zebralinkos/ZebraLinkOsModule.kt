...
class ZebraLinkOsModule(reactContext: ReactApplicationContext) :
    ...
    /**
     * Retorna dispositivos pareados (bonded) Promise.resolve(Array<{name: string, address:
     * string}>)
     */
    @ReactMethod
    ...
    /**
     * Tenta imprimir ZPL em uma impressora Bluetooth pelo MAC (address) Promise.resolve("OK") ou
     * Promise.reject("PRINT_ERROR", message)
     */
    @ReactMethod
    ...
    /**
     * Conecta e retorna basic info (ex.: modelo) - útil para diagnóstico Promise.resolve({ model:
     * string, language: string }) ou reject
     */
    @ReactMethod
    fun connectAndInfo(address: String, promise: Promise) {
        Thread {
                    ...
                    try {
                        ...
                        val model: String =
                                try {
                                    // Algumas versões da Zebra SDK não possuem 'model', então
                                    // usamos "unknown"
                                    printer.javaClass.getMethod("getModel")?.invoke(printer) as?
                                            String
                                            ?: "unknown"
                                } catch (_: Exception) {
                                    "unknown"
                        ...
                        promise.reject(
                                "CONNECT_ERROR",
                                e.message ?: "Erro desconhecido ao conectar"
                        )
                    }
                }
                .start()
